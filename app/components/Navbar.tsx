    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-3 sm:gap-4 group ml-2 sm:ml-6 lg:ml-10">
          ...
          />
          <span className={`${isCongresso ? 'block' : 'hidden sm:block'} w-px h-8 lg:h-10 bg-line shrink-0`} />
          <img
            src="/parceiros/uepa.png"
            alt="Universidade do Estado do Pará"
            className={`${isCongresso ? 'block' : 'hidden sm:block'} h-8 lg:h-11 w-auto object-contain opacity-75 group-hover:opacity-100 transition-opacity -translate-y-1`}
          />
        </Link>
